 ...
 * 识别隐含语义
 */
function extractImplicitSemantics(title: string): string[] {
 ...
 * 判断日程是否暗示焦虑或压力
 */
function detectStressIndicators(title: string, description?: string): { level: 'high' | 'medium' | 'low', reasons: string[] } {
...
export interface HealthReport {
  ...
  suggestions: string[]
  trends: string[]
  recoveryDays: number // 距离上次完全休息的天数
}

export async function checkHealth(events: CalendarEvent[], days: number = 7): Promise<HealthReport> {
  const now = new Date()
  ...
  for (const event of events) {
    if (event.startTime >= startDate) {
      ...
      else if (title.includes('学习') || title.includes('健身') || title.includes('运动') || title.includes('教练') || title.includes('休息')) category = 'personal'
      ...
      // 统计异常时间
      if (hour >= 20 || hour < 6) eveningEvents++
      if (dayOfWeek === 0 || dayOfWeek === 6) weekendEvents++
    }
  }
  ...
  if (!categoryCount['product'] && totalEvents > 3) missingCategories.push('产品')
  if (!categoryCount['team'] && totalEvents > 3) missingCategories.push('团队')
  if (!categoryCount['legal'] && totalEvents > 5) missingCategories.push('法务')
  if (!categoryCount['finance'] && totalEvents > 5) missingCategories.push('财务')
  
  if (missingCategories.length > 0) {
    issues.push('⚠️ 本周缺少：' + missingCategories.join('、') + '相关事务')
    for (const cat of missingCategories) {
      if (cat === '产品') suggestions.push('💡 建议每周至少安排一次产品评审，保持对产品方向的把控')
      if (cat === '团队') suggestions.push('💡 建议每周与核心团队成员进行 1:1 沟通')
      if (cat === '法务') suggestions.push('💡 建议每月审阅一次合同和股权事务')
      if (cat === '财务') suggestions.push('💡 建议每周关注现金流，确保公司健康发展')
    }
  }
